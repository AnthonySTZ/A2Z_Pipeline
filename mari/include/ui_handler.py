...
class ImportMari(QDialog):
    ...
    def init_ui(self) -> None:
        ...
        self.ui.pb_import.clicked.connect(self.import_mesh)
    ...
    def update_name_list(self) -> None:
        ...
        if selected_shot == "":
            ...
            return
        models_path = (
            self.project.path + "/30_assets/" + selected_shot + "/MODEL/EXPORT"
        )
        if not os.path.exists(models_path):
            self.ui.cb_name.clear()
            return
        models = os.scandir(models_path)
        for model in models:
            self.ui.cb_name.addItem(model.name)
    ...
    def import_mesh(self) -> None:
        ...
        if not os.path.exists(mesh_path):
            QMessageBox.information(self, "Error", "Mesh file not found")
            return
        project_name = selected_name[: -selected_name[::-1].find(".")] + "_v001"
        mari.ProjectManager().create(project_name, mesh_path, [], [], {})
        self.close()
